...
function checkPassword() {
	//Check password to see if it matches the password policy
	//Also check retyped password to see if it matches the first one
	...
	if (checkOutputHtml.length > 0 || pass != retypedPass) {
		document.getElementById('submit-registration-button').disabled = true;
	} else {
		document.getElementById('submit-registration-button').disabled = false;
	}
...
if (formInput != 'None') {
	...
	document.getElementById('email').value = formInput.email;
...
document.addEventListener('DOMContentLoaded', function() {
	retyped_password_input = document.getElementById("retyped_password");
